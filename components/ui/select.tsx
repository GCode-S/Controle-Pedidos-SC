  ...
  return (
    <SelectPrimitive.Trigger
      ...
      className={cn(
        ...
        'h-11 text-base data-[size=sm]:h-10 sm:h-9 sm:text-sm sm:data-[size=sm]:h-8',
        className,
      )}
      {...props}
    >
...
function SelectItem({
  ...
  return (
    <SelectPrimitive.Item
      ...
      className={cn(
        "focus:bg-accent focus:text-accent-foreground [&_svg:not([class*='text-'])]:text-muted-foreground relative flex w-full cursor-default items-center gap-2 rounded-sm pr-8 pl-2 outline-hidden select-none data-[disabled]:pointer-events-none data-[disabled]:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 *:[span]:last:flex *:[span]:last:items-center *:[span]:last:gap-2",
        'py-2.5 text-base sm:py-1.5 sm:text-sm',
        className,
      )}
      {...props}
    >
